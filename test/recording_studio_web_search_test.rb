...
class RecordingStudioWebSearchTest < Minitest::Test
  ...
  def test_engine_exists
    assert_kind_of Class, ::RecordingStudio::WebSearch::Engine
  end

  def test_no_flat_compatibility_alias
    refute defined?(RecordingStudioWebSearch)
  end
  ...
  def test_template_does_not_ship_copied_core_hooks_or_base_service
    refute File.exist?(File.expand_path("../lib/recording_studio_web_search/hooks.rb", __dir__))
    refute File.exist?(File.expand_path("../lib/recording_studio_web_search/services/base_service.rb", __dir__))
    refute File.exist?(File.expand_path("../lib/recording_studio_web_search/services/example_service.rb", __dir__))
  end

  def test_does_not_ship_example_capability_or_sample_pages
    refute File.exist?(File.expand_path("../lib/recording_studio_web_search/capabilities/example.rb", __dir__))
    pages = "../db/migrate/20250101000001_create_recording_studio_web_search_pages.rb"
    refute File.exist?(File.expand_path(pages, __dir__))
    refute File.exist?(File.expand_path("../lib/recording_studio.rb", __dir__))
  end
  ...
  def test_recording_studio_keeps_strict_recordable_declarations_enabled
    ...
    recordable_types = "config.recordable_types = [ \"Workspace\", \"Folder\", \"Page\", \"AdminRoot\" ]"
    assert_includes initializer_source, recordable_types
  ...
  def test_dummy_readme_explains_dummy_app_purpose
    ...
    assert_includes readme_source, "This Rails app exists to validate Recording Studio Web Search"
    assert_includes readme_source, "/recording_studio"
    assert_includes readme_source, "redirects to `/`"
    refute_includes readme_source, "flat_pack_sidebar"
  end

  def test_product_readme_covers_search_api
    readme = File.read(File.expand_path("../README.md", __dir__))

    assert_includes readme, "RecordingStudio::WebSearch.search"
    assert_includes readme, "config.provider = :brave"
    assert_includes readme, "search.recording_studio_web_search"
    assert_includes readme, "admin@admin.com"
    assert_includes readme, "web_search"
    refute_includes readme, "Featured In"
    refute_includes readme, "ExampleService"
    refute_includes readme, "RecordingStudio v3"
  end

  def test_dummy_home_page_is_signed_in_search_form
    view_path = File.expand_path("dummy/app/views/home/index.html.erb", __dir__)
    view_source = File.read(view_path)
    controller = File.read(File.expand_path("dummy/app/controllers/home_controller.rb", __dir__))

    assert_includes view_source, 'title: "Web search"'
    assert_includes view_source, "FlatPack::SearchInput::Component"
    assert_includes view_source, 'name: "q"'
    assert_includes view_source, 'label: "Query"'
    assert_includes view_source, 'text: "Search"'
    ...
    assert_includes view_source, "method: :get"
    refute_includes view_source, "method: :post"
    refute_includes view_source, "FlatPack::Breadcrumb::Component"
    assert_includes controller, "authenticate_user!" unless controller.include?("ApplicationController")
    assert_includes controller, "return if @query.blank?"
    assert_includes controller, "RecordingStudio::WebSearch.search"
  end
  ...
  def test_engine_does_not_ship_a_home_view
    view_path = File.expand_path("../app/views/recording_studio_web_search/home/index.html.erb", __dir__)
